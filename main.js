...
`),this.rProcesses.set(e,s),console.log(`R process started and stored for note: ${e}`),s}async runRCodeInSession(e,t,n,s,l){console.log("runRCodeInSession called for note:",e,"with ID:",s);let c=this.getRProcess(e);return new Promise(async(w,o)=>{let a="",i="",d=await ve(L.join(ie.tmpdir(),"rplots-")),m=d.replace(/\\/g,"/"),y=L.join(m,`help_${s}.txt`),p=y.replace(/\\/g,"/"),x=`__END_OF_OUTPUT__${Date.now()}__`,v="__PLOT_PATH__",R=`__ENVIRONMENT_DATA__${Date.now()}__`,C="__WIDGET_PATH__",g=`
library(evaluate)
library(jsonlite)
library(htmlwidgets)
...
`,g);let Q=async H=>{var G,J;let F=H.toString();if(console.log("Received data chunk:",F),a+=F,a.includes(x)){console.log("Marker detected in R output"),c.stdout.off("data",Q),c.stderr.off("data",Y);let P="",V="";if(l){try{let b=await k.promises.readFile(y,"utf8");console.log("Read help content:",b),V=b}catch(b){console.error("Failed to read help content:",b),V="Failed to retrieve help content."}let r=(G=this.app.workspace.getLeavesOfType(D)[0])==null?void 0:G.view;r?r.updateHelpContent(V):console.log("RHelpView not found in the workspace")}let W="";if(a.includes(R)){let r=a.split(R);P=r[0].trim(),W=r[1].split(x)[0].trim()}else P=a.split(x)[0].trim();console.log("Result before processing:",P),console.log("Environment data:",W);let q=v.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),S=[],re=new RegExp(`${q}(.*)`,"g"),N;for(;(N=re.exec(P))!==null;)if(console.log("Image regex match:",N),N[1]){let r=N[1].trim();S.push(r)}else console.error("No image file name captured in regex match:",N);P=P.replace(new RegExp(`${q}.*`,"g"),"").trim();for(let r of S){let b=L.join(d,r);try{let E=await k.promises.readFile(b),_=`plots/${r}`;this.app.vault.getAbstractFileByPath("plots")||(await this.app.vault.createFolder("plots"),console.log('Created "plots" folder in the vault'));let A=this.app.vault.getAbstractFileByPath(_);A?(await this.app.vault.modifyBinary(A,E),console.log(`Image file updated in vault: ${_}`)):(await this.app.vault.createBinary(_,E),console.log(`Image file created in vault: ${_}`)),S[S.indexOf(r)]=_}catch(E){console.error(`Error handling image file ${r}:`,E)}}let T=[],K=C.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),le=new RegExp(`${K}(.*)`,"g"),I;for(;(I=le.exec(P))!==null;)if(I[1]){let r=I[1].trim();T.push(r)}P=P.replace(new RegExp(`${K}.*`,"g"),"").trim();for(let r of T){let b=L.join(d,r);try{let E=await k.promises.readFile(b,"utf8"),_=`widgets/${r}`,Z=this.app.vault.adapter.getBasePath();this.app.vault.getAbstractFileByPath("widgets")||await this.app.vault.createFolder("widgets");let ee=this.app.vault.getAbstractFileByPath(_);ee?await this.app.vault.modify(ee,E):await this.app.vault.create(_,E);let ce=L.join(Z,_),pe=(0,oe.pathToFileURL)(ce).href;T[T.indexOf(r)]=pe}catch(E){console.error(`Error handling widget file ${r}:`,E)}}try{console.log(`Temporary directory ${d} removed`)}catch(r){console.error(`Error removing temporary directory ${d}:`,r)}let X=(J=this.app.workspace.getLeavesOfType($)[0])==null?void 0:J.view;if(X){let r=[];try{r=JSON.parse(W),console.log("Parsed environment variables:",r)}catch(b){console.error("Failed to parse environment data JSON:",b)}X.updateEnvironmentData(n,r)}else console.log("REnvironmentView not found in the workspace");a="",i="",i?o(i.trim()):w({result:P,imagePaths:S,widgetPaths:T,helpContent:V})}},Y=H=>{let F=H.toString();console.error("Received error chunk from R:",F),i+=F};c.stdout.on("data",Q),c.stderr.on("data",Y),c.stdin.write(g+`
